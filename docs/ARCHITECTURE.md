...
### 5.2 MyProto 协议头（8 字节，`__attribute__((packed))`）

```
字节偏移:    0       1       2       3       4       5       6       7
     ┌────────────┬───────┬───────┬──────────────┬──────────────┐
     │   magic    │version│ flags │ channel_id   │  data_len    │
     │  (u16, BE) │ (u8)  │ (u8)  │  (u16, BE)   │  (u16, BE)   │
     ├────────────┼───────┼───────┼──────────────┼──────────────┤
     │  0x4D 0x50 │ 0x01  │ 见下表 │   1 - 65535   │   0 - 65535   │
     └────────────┴───────┴───────┴──────────────┴──────────────┘

magic      = 0x4D50 ('M''P' 的 ASCII，大端序) — 帧识别魔数
version    = 0x01 — 协议版本号
flags      = 帧标志位（位掩码）
channel_id = 通道标识符（网络字节序）
data_len   = 有效负载长度（网络字节序），不含 CRC32
```
