...
需要把现场调试中关于“一个配置文件开启多个示例”和“启动多个 `kcp-afpacket` 实例时如何设置 `ethertype`”的结论整理到统一配置文档中，避免后续重复踩同一网卡、同一 EtherType 下多进程串包的问题。

### 变更

- 更新 `docs/CONFIG.md`。
- 在 `ethertype` 章节补充多实例 EtherType 分配规则。
- 说明同一台机器、同一块网卡上多个实例必须使用不同 `ethertype`，B/C 对端实例的 `ethertype` 必须一致。
- 给出 `35013`/`35014`/`35015`/`35016` 与 `0x88C5`/`0x88C6`/`0x88C7`/`0x88C8` 的示例映射。
- 在 `channels` 章节补充一个配置文件开启多个通道的 JSON 示例。
- 补充端口范围写法、展开结果和注意事项。
- 明确推荐优先使用单进程多通道；确需多进程时必须为每个实例分配独立 `ethertype`。

### 验证

本次仅更新文档，无代码变更，无需重新编译或执行测试。

## 2026-06-05 12:27 补充性能默认值资源影响说明

Commit: `c34af22`

### 背景

需要明确 `performance` 默认参数是否已经写入代码，以及 `proxy_recv_buf_max` 这类高吞吐缓冲参数对其它功能和系统资源的影响。

### 变更

- 更新 `docs/PERFORMANCE_CONFIG.md`。
- 补充说明：不配置 `performance` 时，程序仍使用代码内置默认值。
- 补充 `proxy_recv_buf_max` 的内存占用模型：实际占用取决于写阻塞连接数和每个连接的实际积压数据量。
- 给出 `16 MiB` 默认值下 1/10/100 个写阻塞连接的理论上限示例。
- 补充对 SSH、小流量 TCP、iperf3 高吞吐和高并发生产场景的影响说明。
- 补充根据 `max_sessions × proxy_recv_buf_max` 估算最坏内存上限的建议。
...
按服务器 A/B/C/D 拓扑对 `kcp-afpacket` 的 `performance` 参数进行现场 iperf3 压测调试，目标是尽量接近 A 到 D 直连约 10.3 Gbit/s 的吞吐。

### 变更

- 新增 `docs/IPERF_PERFORMANCE_TUNING_20260604.md`。
- 记录 B/C 实际配置路径、启动方式、直连基线、隧道基线、参数矩阵和测试结果。
- 记录 B/C 系统 socket buffer 上限过低、临时 sysctl 调整、网卡 RX drop 观察结果。
- 记录结论：仅通过现有 `performance` 参数无法稳定接近直连吞吐，标准 P1 隧道多数结果在 400-480 Mbit/s，偶发 577 Mbit/s 不稳定复现。
- 给出当前建议保留配置和下一步代码层优化方向。

### 验证

已执行多轮远端 SSH 调参和 `iperf3 -c 192.168.1.198 -P 1 -t 10` 压测，并将结果写入调试记录文档。
